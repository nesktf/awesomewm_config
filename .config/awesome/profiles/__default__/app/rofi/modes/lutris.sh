...
db_file="/home/nikos/.var/app/net.lutris.Lutris/data/lutris/pga.db"
...
if [ -z $ROFI_INFO ]; then
  ...
  if [[ $ROFI_INFO == "lutris" ]]; then
    coproc (flatpak run net.lutris.Lutris >/dev/null 2>&1)
  else
    kdialog --passivepopup "Opening $ROFI_INFO..." --title "rofi-lutris" --icon "lutris_$ROFI_INFO" 5 >/dev/null 2>&1
    coproc (flatpak run net.lutris.Lutris "lutris:rungame/$ROFI_INFO" >/dev/null 2>&1)
  fi
  exit 0
fi
